...
const profiles: Record<Profile, { runs: number; mint: number; trades: number; gatheringHours: number }> = {
  casual: { runs: 0.7, mint: 42, trades: 0.08, gatheringHours: 0.8 }, active: { runs: 2.2, mint: 49, trades: 0.35, gatheringHours: 1.8 },
  hardcore: { runs: 5.5, mint: 55, trades: 0.7, gatheringHours: 3 }, trader: { runs: 1.1, mint: 44, trades: 3.5, gatheringHours: 1.2 },
  'profession-focused': { runs: 1.6, mint: 38, trades: 1.1, gatheringHours: 4 },
}
...
function simulate(population: number, days: number) {
  ...
  let minted = 0, burned = 0, trades = 0, guildBurn = 0, gatheringResources = 0, riftResources = 0
  for (let day = 0; day < days; day += 1) for (const player of players) {
    ...
    gatheringResources += model.gatheringHours * 10.5
    riftResources += model.runs * 2.45
  ...
  return { population, days, minted, burned, net: minted - burned, inflation: minted ? (minted - burned) / minted : 0, median: percentile(wallets, 0.5), p90: percentile(wallets, 0.9), gini: gini(wallets), velocity: trades / Math.max(1, population * days), feeBurn: burned - guildBurn, guildBurn, coinsPerPlayerDay: (minted - burned) / population / days, gatheringShare: gatheringResources / (gatheringResources + riftResources) }
}

const results = [10, 100, 1_000].flatMap((population) => [1, 7, 30, 90].map((days) => simulate(population, days)))
const rows = results.map((r) => `| ${r.population.toLocaleString()} | ${r.days} | ${r.minted.toLocaleString()} | ${r.burned.toLocaleString()} | ${r.net.toLocaleString()} | ${(r.inflation * 100).toFixed(1)}% | ${r.median} | ${r.p90} | ${r.gini.toFixed(3)} | ${r.velocity.toFixed(2)} | ${r.feeBurn} | ${r.guildBurn} | ${r.coinsPerPlayerDay.toFixed(1)} | ${(r.gatheringShare * 100).toFixed(1)}% |`).join('\n')
